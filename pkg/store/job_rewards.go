...
func (s *KVStore) GetAllUnsettledJobRewards(ctx context.Context, rewardType string) (map[string][]*JobRewardRecord, error) {
	...
	contributors, err := s.ListContributorsWithBalance(ctx, rewardType)
	if err != nil {
	...
	for _, contributor := range contributors {
		records, err := s.GetJobRewardsSinceLastSettlement(ctx, contributor.WalletAddress, rewardType)
		if err != nil {
			slog.Warn("Failed to get job rewards for contributor", 
				"address", contributor.WalletAddress, "error", err)
			continue
		}
		
		if len(records) > 0 {
			result[contributor.WalletAddress] = records
		}
	}
