...
func fetchRange(ctx context.Context, client *http.Client, url string, start, end int64) ([]byte, int64, error) {
	...
	if token := os.Getenv("VERIDIUM_HF_TOKEN"); token != "" {
	...
	var fileSize int64
	// Try to parse Content-Range header first
	parsed := false
	if cr != "" {
		var start, end int64
		if _, err := fmt.Sscanf(cr, "bytes %d-%d/%d", &start, &end, &fileSize); err == nil {
			parsed = true
		}
	}

	// If parsing failed or no Content-Range, fall back to Content-Length
	if !parsed && resp.ContentLength > 0 && resp.StatusCode == http.StatusOK {
...
func readMetadataValueFromReader(r *bytes.Reader, valueType uint32) (interface{}, error) {
	...
	case ggufMetadataValueTypeArray:
		...
		if arrayLen > 1024*1024 {
			return nil, fmt.Errorf("array length too large: %d", arrayLen)
		}
