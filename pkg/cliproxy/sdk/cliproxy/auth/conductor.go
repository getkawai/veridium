...
import (
	...
	"github.com/kawai-network/veridium/pkg/cliproxy/internal/util"
...
func (m *Manager) executeWithProvider(ctx context.Context, provider string, req cliproxyexecutor.Request, opts cliproxyexecutor.Options) (cliproxyexecutor.Response, error) {
	...
	for {
		...
		if accountType == "api_key" {
			if proxyInfo != "" {
				logger.Debug("Use API key for model", "key", util.HideAPIKey(accountInfo), "model", req.Model, "proxy", proxyInfo)
			} else {
				logger.Debug("Use API key for model", "key", util.HideAPIKey(accountInfo), "model", req.Model)
			}
		} else if accountType == "oauth" {
			if proxyInfo != "" {
				logger.Debug("Use OAuth for model", "account", accountInfo, "model", req.Model, "proxy", proxyInfo)
			} else {
				logger.Debug("Use OAuth for model", "account", accountInfo, "model", req.Model)
			}
		}
...
func (m *Manager) checkRefreshes(ctx context.Context) {
	...
	for _, a := range snapshot {
		...
		if typ != "api_key" {
			...
			slog.Debug("checking refresh for auth", "provider", a.Provider, "id", a.ID, "type", typ)
...
func (m *Manager) refreshAuth(ctx context.Context, id string) {
	...
	if err != nil {
		slog.Debug("refreshed auth with error", "provider", auth.Provider, "id", auth.ID, "error", err)
	} else {
		slog.Debug("refreshed auth successfully", "provider", auth.Provider, "id", auth.ID)
	}
	now := time.Now()
	if err != nil {
		m.mu.Lock()
...
// loggerWithRequestID returns a slog logger with request_id attribute if available in context.
