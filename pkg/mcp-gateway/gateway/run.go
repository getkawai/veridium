...
import (
	...
	"github.com/kawai-network/veridium/pkg/mcp-gateway/health"
	"github.com/kawai-network/veridium/pkg/mcp-gateway/interceptors"
	...
	"github.com/kawai-network/veridium/pkg/mcp-gateway/user"
)
...
func (g *Gateway) periodicMetricExport(ctx context.Context) {
	...
	for {
		...
		case <-ticker.C:
			...
			if os.Getenv("DOCKER_MCP_TELEMETRY_DEBUG") != "" {
				fmt.Fprintf(os.Stderr, "[MCP-TELEMETRY] Periodic metric flush (no-op)\n")
			}
		}
	}
}
