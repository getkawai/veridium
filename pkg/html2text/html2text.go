...
// PrettyTablesOptions configures custom table rendering
...
// renderTable renders a table using custom ASCII formatting
func renderTable(header []string, body [][]string, footer []string, options *PrettyTablesOptions) string {
	if options == nil {
	...
	colWidths := calculateColumnWidths(header, body, footer)

	// Render header
	if len(header) > 0 {
		renderRow(&buf, header, colWidths, options.ColumnSeparator)
		if options.HeaderLine {
			renderSeparatorLine(&buf, colWidths, options.RowSeparator, options.ColumnSeparator)
		}
	}

	// Render body rows
	for i, row := range body {
		renderRow(&buf, row, colWidths, options.ColumnSeparator)
		if options.RowLine && i < len(body)-1 {
			renderSeparatorLine(&buf, colWidths, options.RowSeparator, options.ColumnSeparator)
		}
	}
	...
	if len(footer) > 0 {
		...
		renderRow(&buf, footer, colWidths, options.ColumnSeparator)
	}
...
func calculateColumnWidths(header []string, body [][]string, footer []string) []int {
	maxCols := len(header)
...
func renderRow(buf *bytes.Buffer, row []string, colWidths []int, separator string) {
	for i, cell := range row {
		if i > 0 {
...
// renderSeparatorLine renders a separator line between table rows
func renderSeparatorLine(buf *bytes.Buffer, colWidths []int, rowSep, colSep string) {
	for i, width := range colWidths {
		if i > 0 {
			// Replace separator characters with row separator
			sepReplacement := strings.Repeat(rowSep, len(colSep))
			buf.WriteString(sepReplacement)
		}
		buf.WriteString(strings.Repeat(rowSep, width))
	}
	buf.WriteString("\n")
}
...
func (ctx *textifyTraverseContext) handleTableElement(node *html.Node) error {
	...
	case atom.Table:
		...
		// Render the table using custom ASCII formatting
		tableOutput := renderTable(ctx.tableCtx.header, ctx.tableCtx.body, ctx.tableCtx.footer, ctx.options.PrettyTablesOptions)
		if err := ctx.emit(tableOutput); err != nil {
			return err
		}
