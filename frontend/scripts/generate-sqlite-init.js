#!/usr/bin/env node
/**
 * Generate initial SQLite migration from the updated schemas
 * This creates the initial migration that can be used to set up a fresh SQLite database
 */
...
const outputDir = path.join(__dirname, '../src/database/migrations');
...
const schemaFiles = fs.readdirSync(schemasDir)
  .filter(file => file.endsWith('.ts') && file !== 'index.ts' && file !== '_helpers.ts');

const migrationSQL = `-- Initial SQLite migration generated from schemas
...
INSERT OR IGNORE INTO drizzle.__drizzle_migrations (hash, created_at) VALUES ('initial_sqlite_setup', strftime('%s', 'now') * 1000);`;

// Write the SQL to a file
const outputFile = path.join(outputDir, '0000_initial_sqlite_setup.sql');
fs.writeFileSync(outputFile, migrationSQL, 'utf8');

console.log(`✅ Generated SQLite migration file: ${outputFile}`);
console.log(`📊 Total tables created: ${schemaFiles.length + 5}`); // +5 for junction tables etc.