...
export interface ModelProcessorConfig {
  excludeKeywords?: readonly string[]; // Do not add tags to matching models
...
// Default keywords: Any model ID containing -search is considered to support web search
...
/**
 * Detect whether keyword list matches model ID (supports multiple matching modes)
 ...
 *   - no prefix: include match (default behavior)
 * @returns Whether it matches (exclude logic takes priority)
 */
const isKeywordListMatch = (modelId: string, keywords: readonly string[]): boolean => {
  // First check exclude rules (starting with exclamation mark)
  ...
  // If any exclude rule matches, return false directly
...
const formatTimestampToDate = (timestamp: number): string | undefined => {
  ...
  // - If it's millisecond level (>= 1e12), treat as milliseconds directly;
  // - Otherwise treat as seconds, need *1000 to convert to milliseconds
...
const processReleasedAt = (model: any, knownModel?: any): string | undefined => {
  // Prioritize checking model.created
  if (model.created !== undefined && model.created !== null) {
    ...
    if (typeof model.created === 'number' && model.created > 1_630_000_000) {
      // AiHubMix error timestamp is 1626777600
      return formatTimestampToDate(model.created);
    }
    // If created is a string and is already in date format, return directly
...
/**
 * Get local configuration of model provider
 * @param provider Model provider
 * @returns Local configuration of model provider
 */
 ...
 * @param providerLocalConfig Local configuration of model provider
 * @param model Model object
...
/**
 * Process general logic of model card
 */
...
/**
 * Process model list of single provider
 * @param modelList Model list
 * @param config Provider configuration
 * @param provider Provider type (optional, used to prioritize matching corresponding local configuration, when provider is provided, will try to override enabled from local configuration)
...
export const processModelList = async (
  ...
  // If provider is provided, try to get the local configuration of that provider
  ...
  return Promise.all(
    modelList.map(async (model) => {
      ...
      // If not found, fallback to global configuration
      if (!knownModel) {
      ...
      // If the model in local configuration is found, use its enabled status
      if (
        processedModel &&
...
/**
 * Process model list of mixed providers
 * @param modelList Model list
...
export const processMultiProviderModelList = async (
  ...
  // If providerid is provided, try to get the local configuration of that provider
