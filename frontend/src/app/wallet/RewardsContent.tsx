...
import { Coins, Award, Users, RefreshCw } from 'lucide-react';
...
const RewardsContent = ({ styles, theme, currentNetwork, transactions, setModalType }: RewardsContentProps) => {
  const [activeTab, setActiveTab] = useState<'mining' | 'cashback' | 'referral'>('mining');
  ...
  return (
    <Flexbox style={{ maxWidth: 1200, width: '100%' }} gap={20}>
      ...
      <Tabs
        activeKey={activeTab}
        onChange={(key) => setActiveTab(key as 'mining' | 'cashback' | 'referral')}
        size="large"
        items={[
          {
            key: 'mining',
