// import { LobeChatPluginManifest, Meta } from '@/chat-plugin-sdk';

import { LobeToolType } from './tool';

export type PluginManifestMap = Record<string, any>; // LobeChatPluginManifest>;
...
export interface LobeToolCustomPlugin {
  ...
  identifier: string;
  manifest?: any; // LobeChatPluginManifest;
  settings?: any;
  type: 'customPlugin';
}

export interface InstallPluginMeta extends Partial<any> { // Partial<Meta>
  author?: string;
  createdAt?: string;
  homepage?: string;
  identifier: string;
