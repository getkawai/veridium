...
import { Buffer } from './node-compat';
import yauzl from 'yauzl';

// Define basic error messages
const ERRORMSG = {
  fileCorrupted: (filepath: string | InstanceType<typeof Buffer>) =>
 ...
 * @param {InstanceType<typeof Buffer>|string} zipInput ZIP file input, either a Buffer or a file path (string).
...
export function extractFiles(
  zipInput: InstanceType<typeof Buffer> | string,
  ...
  return new Promise((resolve, reject) => {
    ...
    if (zipInput instanceof Buffer) {
      // Convert our Buffer to Uint8Array for yauzl
      try {
        const bufferData = atob(zipInput.toBytes());
        const uint8Array = new Uint8Array(bufferData.length);
        for (let i = 0; i < bufferData.length; i++) {
          uint8Array[i] = bufferData.charCodeAt(i);
        }

        // Process ZIP from Buffer
        yauzl.fromBuffer(uint8Array, { lazyEntries: true }, (err, zipfile) => {
          if (err || !zipfile) return reject(err || new Error('Failed to open zip from buffer'));
          processZipfile(zipfile);
        });
      } catch (error) {
        reject(new Error(`Failed to decode buffer: ${error}`));
      }
