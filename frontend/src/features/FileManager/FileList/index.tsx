...
const FileList = memo<FileListProps>(({ knowledgeBaseId, category, onOpenFile }) => {
  ...
  const [query, sorter, sortType, isFetchingFiles, fileList] = useFileStore((s) => [
    fileManagerSelectors.searchKeywords(s),
    fileManagerSelectors.sorter(s),
    fileManagerSelectors.sortType(s),
    fileManagerSelectors.isFetchingFiles(s),
    fileManagerSelectors.fileList(s),
  ]);

  const fetchFileList = useFileStore((s) => s.fetchFileList);
  ...
  }, [category, knowledgeBaseId, query, sortType, sorter, viewConfig, fetchFileList]);

  const isLoading = isFetchingFiles;
  const data = fileList;
