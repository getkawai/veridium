...
// chromemStore implements a simple vector store using chromem-go with HuggingFace embedder
...
func newChromemStore(ctx context.Context, dbPath, collectionName string, embedder *embeddings.EmbedderImpl) (*chromemStore, error) {
	// Create embedding function using the HuggingFace embedder
...
func newStore() (*chromemStore, error) {
	// Create OpenAI client with local HuggingFace TEI endpoint
	...
	tempDir := filepath.Join(os.TempDir(), "chromem-huggingface-example")
	os.MkdirAll(tempDir, 0755)

	ctx := context.Background()

	// Create a new chromem vector store with HuggingFace embeddings
	store, err := newChromemStore(ctx, tempDir, "huggingface-example", embedder)
	if err != nil {
		return nil, err
	}
...
func citiesExample(store *chromemStore) {
