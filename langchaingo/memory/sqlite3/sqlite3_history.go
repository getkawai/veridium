...
// AddMessages adds multiple messages to the chat message history in a single transaction.
func (h *SqliteChatMessageHistory) AddMessages(ctx context.Context, messages []llms.ChatMessage) error {
	if len(messages) == 0 {
		return nil
	}

	// Begin transaction for bulk insert
	buf := bytes.NewBufferString("BEGIN TRANSACTION; INSERT INTO ")
