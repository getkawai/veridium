...
func (s *TradeService) storePartialTradeRecord(order *Order, txHash string, tradeTokenAmount, tradeUSDTAmount *big.Int, buyerAddress string) error {
	...
	// Add trade to seller's trade history (Priority 2)
	err = s.addTradeToUserHistory(order.Seller, "seller", tradeID)
	if err != nil {
		return fmt.Errorf("failed to add trade to seller history: %w", err)
	}

	// Add trade to buyer's trade history (Priority 2)
	err = s.addTradeToUserHistory(buyerAddress, "buyer", tradeID)
	if err != nil {
		return fmt.Errorf("failed to add trade to buyer history: %w", err)
	}
