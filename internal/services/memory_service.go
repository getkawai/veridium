...
func (s *MemoryService) CreateMemory(ctx context.Context, memory *Memory) (*Memory, error) {
	...
	if memory.Summary != "" {
		...
		if err != nil {
			slog.Warn("Failed to generate summary embedding", "error", err)
	...
	if memory.Details != "" {
		...
		if err != nil {
			slog.Warn("Failed to generate details embedding", "error", err)
		} else if len(embeddings) > 0 {
	...
	slog.Info("Memory created", "memory_id", memory.ID, "category", memory.Category, "type", memory.Type)
...
func (s *MemoryService) SemanticSearch(ctx context.Context, query string, limit int) ([]*MemorySearchResult, error) {
	...
	if s.duckDB != nil {
		...
		if err != nil {
			slog.Warn("DuckDB search failed, falling back to SQLite", "error", err)
...
func (s *MemoryService) ArchiveOldMemories(ctx context.Context, olderThanDays int) error {
	...
	slog.Info("Archived memories", "older_than_days", olderThanDays)
	return nil
}
