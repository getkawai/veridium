...
const (
	ProviderLlama      ProviderType = "llama"      // Local llama.cpp
	ProviderOpenAI     ProviderType = "openai"     // OpenAI API
	ProviderOpenRouter ProviderType = "openrouter" // OpenRouter (OpenAI-compatible)
	ProviderZhipuAI    ProviderType = "zhipu"      // Zhipu GLM (OpenAI-compatible)
	ProviderOllama     ProviderType = "ollama"     // Ollama (OpenAI-compatible)
	ProviderGroq       ProviderType = "groq"       // Groq (OpenAI-compatible)
)
...
type ProviderConfig struct {
	...
	BaseURL   string         `json:"base_url"`   // API endpoint (empty for local)
	APIKey    string         `json:"api_key"`    // API key (empty for local)
