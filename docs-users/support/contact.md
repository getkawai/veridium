# Contact Support

Need help? We're here for you! Choose the best way to reach us:

## 🚀 Quick Help

### In-App Support
The fastest way to get help:

1. Open Kawai desktop app
2. Click **Help** in the menu
3. Select **Contact Support**
4. Describe your issue
5. Get response within 24 hours

## 💬 Community Support

### Discord Server
Join our active community:

- **General Help**: #support channel
- **Technical Issues**: #tech-support
- **Feature Requests**: #suggestions
- **Community Chat**: #general

[Join Discord →](https://discord.gg/SNf3ZEa8Eq)

### Telegram Group
Quick questions and updates:

- Real-time community help
- Official announcements
- Network with other users

[Join Telegram →](https://t.me/kawai_network)

## 📧 Direct Contact

### Email Support
For detailed issues or private matters:

**support@kawai.network**

Response time: 24-48 hours

Please include:
- Your wallet address (first 6 and last 4 characters)
- Description of the issue
- Screenshots if applicable
- Steps to reproduce (for bugs)

### Business Inquiries
For partnerships and business:

**business@kawai.network**

## 🐛 Report Bugs

### GitHub Issues
For technical bugs and feature requests:

1. Visit [GitHub Repository](https://github.com/kawai-network/veridium)
2. Click **Issues** tab
3. Search existing issues first
4. Create new issue with details

### Bug Bounty
Found a security vulnerability?

**security@kawai.network**

We reward responsible disclosure!

## 📚 Self-Help Resources

Before contacting support, try these:

### Documentation
- [Getting Started Guide](../getting-started/quick-start.md)
- [Rewards Overview](../rewards/overview.md)
- [FAQ](../faq/general.md)
- [User Guide](../user-guide/wallet-setup.md)

### Video Tutorials
- YouTube channel (coming soon)
- In-app video guides

### Knowledge Base
- Common issues and solutions
- Step-by-step tutorials
- Best practices

## 🕐 Support Hours

### Community Support (Discord/Telegram)
- **24/7** - Community members worldwide

### Official Support (Email/In-App)
- **Monday-Friday**: 9 AM - 6 PM UTC
- **Weekend**: Limited support
- **Response time**: 24-48 hours

### Emergency Support
For critical issues (security, lost funds):
- Email: **emergency@kawai.network**
- Response time: 2-4 hours

## 📋 Before You Contact

Help us help you faster! Please have ready:

### For Technical Issues
- [ ] Wallet address (public)
- [ ] Operating system and version
- [ ] App version (Help → About)
- [ ] Error message or screenshot
- [ ] Steps to reproduce

### For Reward Issues
- [ ] Wallet address
- [ ] Transaction hash (if applicable)
- [ ] Settlement period
- [ ] Expected vs actual amount

### For Account Issues
- [ ] Wallet address
- [ ] Last successful login date
- [ ] Description of the problem

## 🌟 Feedback

We love hearing from you!

### Feature Requests
- Discord #suggestions channel
- GitHub feature request
- Email: feedback@kawai.network

### General Feedback
- In-app feedback form
- Community surveys
- Social media

## 🔗 Social Media

Stay connected:

- **Twitter**: [@KawaiNetwork](https://twitter.com/kawainetwork)
- **Discord**: [Join Server](https://discord.gg/SNf3ZEa8Eq)
- **Telegram**: [Join Group](https://t.me/kawai_network)
- **GitHub**: [View Code](https://github.com/kawai-network/veridium)
- **Medium**: [Read Blog](https://medium.com/@kawainetwork)

## ⚠️ Important Notes

### Never Share
- ❌ Private keys
- ❌ Seed phrases
- ❌ Passwords
- ❌ 2FA codes

**Official support will NEVER ask for these!**

### Beware of Scams
- ✅ Official domain: kawai.network
- ✅ Official email: @kawai.network
- ❌ Fake support accounts on social media
- ❌ Phishing websites

### Verify Official Channels
Always verify you're contacting official Kawai support:
- Check domain names carefully
- Look for verified badges
- Cross-reference with official website

---

**Need immediate help?** [Join Discord](https://discord.gg/SNf3ZEa8Eq) for fastest community response!
