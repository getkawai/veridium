...
func generateCloudflareGoFile(vars []ConfigVar, configs map[string]string) string {
	...
	sb.WriteString("import \"github.com/kawai-network/x/env\"\n\n")
	...
	if hasCredentials {
		...
		for _, v := range vars {
			if !v.IsNamespace {
				...
				encoded := env.EncodeString(value)
	...
	for _, v := range vars {
		...
		sb.WriteString(fmt.Sprintf("\tval, _ := env.DecodeString(obfuscated%s)\n", v.GoName))
...
func generateEtherscanGoFile(keys []string) string {
	...
	for i := range keys {
		...
		sb.WriteString(fmt.Sprintf("\tval, _ := env.DecodeString(obfuscatedEtherscanApiKey%d)\n", i))
		sb.WriteString("\treturn val\n")
		sb.WriteString("}\n\n")
	}
...
func generateTreasuryGoFile(addresses []string) string {
	...
	for i, address := range addresses {
		encoded := env.EncodeString(address)
	...
	for i := range addresses {
		...
		sb.WriteString(fmt.Sprintf("\tval, _ := env.DecodeString(obfuscatedTreasuryAddress%d)\n", i))
		sb.WriteString("\treturn val\n")
		sb.WriteString("}\n\n")
	}
...
func generateLLMGoFile(openrouterKeys, zaiKeys, geminiKeys, hfKeys, cloudflareKeys []string) string {
	...
	if len(openrouterKeys) > 0 {
		...
		for i := range openrouterKeys {
			...
			sb.WriteString(fmt.Sprintf("\tval, _ := env.DecodeString(obfuscatedOpenRouterApiKey%d)\n", i))
			sb.WriteString("\treturn val\n")
			sb.WriteString("}\n\n")
		}
	}
	...
	if len(zaiKeys) > 0 {
		...
		for i := range zaiKeys {
			...
			sb.WriteString(fmt.Sprintf("\tval, _ := env.DecodeString(obfuscatedZaiApiKey%d)\n", i))
			sb.WriteString("\treturn val\n")
			sb.WriteString("}\n\n")
		}
	}
	...
	if len(geminiKeys) > 0 {
		...
		for i := range geminiKeys {
			...
			sb.WriteString(fmt.Sprintf("\tval, _ := env.DecodeString(obfuscatedGeminiApiKey%d)\n", i))
			sb.WriteString("\treturn val\n")
			sb.WriteString("}\n\n")
		}
	}
	...
	if len(hfKeys) > 0 {
		...
		for i := range hfKeys {
			...
			sb.WriteString(fmt.Sprintf("\tval, _ := env.DecodeString(obfuscatedHfApiKey%d)\n", i))
			sb.WriteString("\treturn val\n")
			sb.WriteString("}\n\n")
		}
	}
	...
	if len(cloudflareKeys) > 0 {
		...
		for i := range cloudflareKeys {
			...
			sb.WriteString(fmt.Sprintf("\tval, _ := env.DecodeString(obfuscatedCloudflareApiKey%d)\n", i))
			sb.WriteString("\treturn val\n")
			sb.WriteString("}\n\n")
		}
	}
