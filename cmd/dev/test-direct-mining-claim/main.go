...
func main() {
	if err := testDirectMiningClaim(); err != nil {
		log.Fatalf("Failed to test direct mining claim: %v", err)
	}
}

func testDirectMiningClaim() error {
	ctx := context.Background()

	fmt.Println("🧪 Direct Mining Claim Test")
	fmt.Println("═══════════════════════════════════════")
	fmt.Println()
	...
	if unclaimedRaw, ok := claimableData["unclaimed_proofs"]; ok {
		if unclaimedList, ok := unclaimedRaw.([]*store.MerkleProofData); ok {
			for _, proof := range unclaimedList {
				if proof.RewardType == "kawai" && proof.PeriodID == 1768136095 {
					testProof = proof
					break
				}
			}
	...
	fmt.Printf("🚀 Submitting Mining Claim Transaction...\n")
	fmt.Printf("   Contract Period:    %d\n", contractPeriod)
	...
	fmt.Printf("🎉 Mining claim successful!\n")
	fmt.Printf("   Block Number: %d\n", receipt.BlockNumber.Uint64())
	fmt.Printf("   Gas Used:     %d\n", receipt.GasUsed)
	fmt.Println()

	fmt.Printf("✅ Direct mining claim test completed successfully!\n")
