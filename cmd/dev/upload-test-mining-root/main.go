...
func main() {
	if err := uploadTestMiningRoot(); err != nil {
		log.Fatalf("Failed to upload test mining root: %v", err)
	}
}

func uploadTestMiningRoot() error {
	ctx := context.Background()

	fmt.Println("🚀 Uploading Test Mining Root")
	fmt.Println("═══════════════════════════════════════")
	fmt.Println()

	// Test settlement details
	testMerkleRoot := "0x7533cad7750276a739f1cf15f8a7d2343ccccbac10be06423086de1c4307fba0"
	testPeriodID := int64(1768135359)

	fmt.Printf("📋 Test Settlement Details:\n")
	...
	fmt.Printf("⚠️  About to advance contract to period %d with test Merkle root\n", currentPeriod.Uint64()+1)
	...
	fmt.Printf("✅ Test mining root uploaded successfully in block %d\n", receipt.BlockNumber.Uint64())
	fmt.Println()
	...
	fmt.Printf("   Settlement Period %d -> Contract Period %d (TEST)\n", testPeriodID, finalPeriod.Uint64())
	fmt.Println()

	fmt.Printf("✅ Test mining root uploaded successfully!\n")
	fmt.Printf("📝 Next: Test claiming with the correct user address\n")
