...
func main() {
	if err := createFinalTreasurySettlement(); err != nil {
		log.Fatalf("Failed to create final treasury settlement: %v", err)
	}
}

func createFinalTreasurySettlement() error {
	ctx := context.Background()

	fmt.Println("🏆 Creating FINAL Treasury Mining Settlement (Period 8)")
	fmt.Println("═══════════════════════════════════════════════════════")
	fmt.Println()
	...
	treasuryAddr := "0x94D5C06229811c4816107005ff05259f229Eb07b"     // Treasury/Developer address
	...
	// Use CONTRACT PERIOD 8 (yang sekarang ada di contract)
	contractPeriod := uint64(8)             // Current contract period
	settlementPeriodID := time.Now().Unix() // For KV storage key

	fmt.Printf("📋 FINAL Treasury Settlement Parameters:\n")
	...
	// Generate Merkle leaves using CONTRACT PERIOD 8
	var leaves [][]byte
	leafMap := make(map[string][]byte) // Map contributor address to leaf hash
	totalAmount := big.NewInt(0)

	for _, reward := range testRewards {
		// Generate 9-field Merkle leaf using CONTRACT PERIOD 8
		leaf := generateFinalTreasuryMerkleLeaf(
			contractPeriod, // Use contract period (8)!
	...
	// Save settlement
	fmt.Println()
	fmt.Printf("💾 Saving FINAL Treasury settlement...\n")
	...
	fmt.Printf("✅ FINAL Treasury settlement created successfully!\n")
	fmt.Println()
	...
	fmt.Printf("   ✅ Root matches what's uploaded to contract\n")
	fmt.Printf("   ✅ All issues resolved!\n")
	fmt.Println()

	fmt.Printf("📝 Next Steps:\n")
	fmt.Printf("   1. Update period mapping to include: %d -> %d\n", settlementPeriodID, contractPeriod)
	fmt.Printf("   2. Test claiming - should work perfectly now!\n")

	return nil
}

// generateFinalTreasuryMerkleLeaf creates a 9-field Merkle leaf using CONTRACT PERIOD 8
// This matches exactly what the contract expects
func generateFinalTreasuryMerkleLeaf(
	contractPeriod uint64, // Use contract period (8), not timestamp
	...
	return crypto.Keccak256(
		common.LeftPadBytes(big.NewInt(int64(contractPeriod)).Bytes(), 32), // uint256 period (CONTRACT PERIOD 8!)
