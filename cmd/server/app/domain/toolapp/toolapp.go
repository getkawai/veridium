...
func (a *app) createToken(ctx context.Context, r *http.Request) web.Encoder {
	...
	if err != nil {
		return errs.Errorf(errs.InvalidArgument, "invalid duration format: %s", err)
	}
	...
	resp, err := a.authClient.CreateToken(ctx, bearerToken, req.Admin, endpoints, duration)
	if err != nil {
		return errs.New(errs.Internal, err)
	}
